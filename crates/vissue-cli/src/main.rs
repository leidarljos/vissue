...
/// are scanned in full. Only `IssueNotFound` falls through; a duplicate id
/// that happens to look like an accession stays a duplicate.
fn backlinks_layouts(router: &Router, id: &str) -> Result<Vec<Layout>> {
    Ok(router.backlinks_layouts(id)?)
}
